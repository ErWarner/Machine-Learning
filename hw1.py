...
print 'The first 3 singular values are:',S[0:3]
...
a,b = A.shape
Sigma3 = np.zeros(shape=(a,b))

Sigma3[0:3,0:3] = Sigma[0:3,0:3]


B = np.mat(U)*np.mat(Sigma3)*np.mat(V)


C = A-B
U2,S2,V2 = np.linalg.svd(C)
SigmaC = np.diag(S2)
Dc     = np.mat(C)*np.transpose(C);
TD     = np.trace(Dc)


Answer = math.sqrt(TD)

print 'The Frobenius norm of A-B is:', Answer
